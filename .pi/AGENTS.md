...
3. **Git:** one commit per feature, staged by path (`git add .` is a fallback, say so when used). Plain `git commit -m` only — no heredocs, no `-F` with inline prose, and never trigger words (`shutdown`, `exec`, `kill`) anywhere in the command line. Never `--no-verify`, `core.hooksPath` or force flags. **Never push without the owner asking.**
