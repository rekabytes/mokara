...
// Privacy Policy — the document body.
//
// Server-rendered prose DATA, not rendering logic: `SECTIONS` is an array of
// plain objects whose `body` is JSX built from the LegalDoc primitives, and the
// "On this page" contents list is generated from this same array, so the TOC
// cannot drift from the sections. `components/LegalDoc.tsx` owns the chrome.
//
// No "use client", no hooks, no motion, no Jotai — and that is load-bearing:
// `@/lib/legal` reads the operator's identity from `process.env` at module
// scope, so pulling this file into a client bundle would inline those env reads
// and blank every `Pending` chip and mailto link. structure.mjs asserts zero
// `'use client'` anywhere under app/(legal).
